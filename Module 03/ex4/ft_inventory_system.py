...
def current_inventory(inventory, total_items):
    print("\n=== Current Inventory ===")
    printed = 0
    ...
    while printed < total_unique:
        ...
        for name, value in inventory.items():
            if name not in used and value > max_value:
                ...
                max_key = name
        if max_value > 1:
            print(f"{max_key}: {max_value} units ({((max_value / total_items) * 100):.1f}%)")
        else:
            print(f"{max_key}: {max_value} unit ({((max_value / total_items) * 100):.1f}%)")
        used[max_key] = True
        printed += 1

def inv_stats(inventory):
...
def dict_demos(inventory):
    ...
    if inventory.get("sword") == None:
        ...
        print("Samplye lookup - 'sword' in inventory: True")
